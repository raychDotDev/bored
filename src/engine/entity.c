...
void EntityUpdate(Entity *self, World *ctx, f32 dt) {
	...
	if (self->on_update)
		self->on_update(self, ctx, dt);
	// self->vel = Vector2Add(self->vel, Vector2Scale(self->fri, dt));
	// self->vel =
	...
	for (i32 i = 0; i < ctx->entities_count; i++) {
		...
		overlap *= 0.5f;
		if (dist < rad_sum) {
			EntityCollideEntity(self, other, dt);
			EntityCollideEntity(other, self, dt);
	...
	if (world_col_normal.x != 0 || world_col_normal.y != 0) {
		...
		EntityCollideWall(self, world_col_normal, dt);
	}
		// Apply frame-rate independent friction
		if (self->friction > 0.0f && !Vector2Equals(self->vel, (v2){0,0})) {
			f32 decay = expf(-self->friction * dt);
			self->vel = Vector2Scale(self->vel, decay);

			// Опционально: обнуляем очень маленькие скорости, чтобы избежать "дрожания"
			if (Vector2Length(self->vel) < 0.01f) {
				self->vel = (v2){0};
			}
		}

		// Гравитация применяется отдельно (она уже frame-rate independent через dt)
		if (self->affected_by_gravity)
			EntityApplyForce(self, ctx->gravity, dt);
}
void EntityCollideEntity(Entity *self, Entity *other, f32 dt) {
	if (!self || !self->on_collide_entity || !other)
		return;
	self->on_collide_entity(self, other, dt);
}
void EntityCollideWall(Entity *self, v2 normal, f32 dt) {
	if (!self || !self->on_collide_wall)
		return;
	self->on_collide_wall(self, normal, dt);
}
